...
namespace JYCMS.WebAPI.DataSQLHelper
{
    /// <summary>
    /// MySqlHelper操作类
    /// </summary>
    public abstract class MySqlHelper
    {
       ...
       // public static string Conn = "Database='device_manage';Data Source='localhost';User Id='root';Password='123456';charset='utf8';pooling=true;Allow Zero Datetime=True";
        public static readonly string ConnectionString = AppConfigurtaionServices.Configuration.GetSection("Appsettings").GetSection("ConnectionString").Value;
        /// <summary>
        ...
        public static DataSet GetDataSetNoP(CommandType cmdType, string cmdText)
        {
            ...
            using (MySqlConnection conn = new MySqlConnection(ConnectionString))
            {
                ...
                {
                    //调用 PrepareCommand 方法，对 MySqlCommand 对象设置参数
                    PrepareCommandNoP(cmd, conn, null, cmdType, cmdText);
                    //调用 MySqlCommand 的 ExecuteReader 方法
                    MySqlDataAdapter adapter = new MySqlDataAdapter();
                    adapter.SelectCommand = cmd;
                    DataSet ds = new DataSet();
        ...
        private static void PrepareCommandNoP(MySqlCommand cmd, MySqlConnection conn, MySqlTransaction trans, CommandType cmdType, string cmdText)
        {
            if (conn.State != ConnectionState.Open)
                conn.Open();
            cmd.Connection = conn;
